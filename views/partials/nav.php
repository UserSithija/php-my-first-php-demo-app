        ...
        <div class="hidden md:block">
          <div class="ml-10 flex items-baseline space-x-4">
            ...
            <?php foreach ($links as $link) : ?>
              <?php
              $isNotes = strtolower($link['name']) === 'notes';
              $hasSession = isset($_SESSION['user']) && $_SESSION['user'] !== null;
              // Show "notes" only if session exists, show all others always
              if (!$isNotes || ($isNotes && $hasSession)) : ?>

                <a href=<?= $link['path'] ?> class="<?= $_SERVER['REQUEST_URI'] === $link['path'] ? "bg-gray-900 text-white" : "text-gray-300 " ?>rounded-md  px-3 py-2 text-sm font-medium  text-white  hover:bg-gray-700 hover:text-white" aria-current="page"><?= $link['name'] ?></a>
            ...
            <div class="flex justify-center align-center gap-3">
              <?php if ($_SESSION['user'] ?? false) : ?>
                ...
                <form action="/sessions" method="POST">
                  ...
                  <button class="mx-3 text-white p-2 bg-gray-950/70 rounded-md">logout</button>
                </form>
              <?php else: ?>
